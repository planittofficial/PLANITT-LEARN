...
export function useAdminLessons(moduleId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/v1/admin/lessons?moduleId=${moduleId}`, withApiCredentials());
...
export function useAdminLesson(lessonId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/v1/admin/lessons/${lessonId}`, withApiCredentials());
...
export function useCreateLesson(moduleId: string) {
  ...
  return useMutation({
    mutationFn: async (body: Record<string, unknown>) => {
      const res = await fetch("/api/v1/admin/lessons", withApiCredentials({
        method: "POST",
...
export function useDeleteLesson(moduleId: string) {
  ...
  return useMutation({
    mutationFn: async (lessonId: string) => {
      const res = await fetch(`/api/v1/admin/lessons/${lessonId}`, withApiCredentials({ method: "DELETE" }));
...
export function usePresignUpload() {
  return useMutation({
    mutationFn: async (body: { filename: string; contentType?: string; lessonId?: string }) => {
      const res = await fetch("/api/v1/admin/uploads/presign", withApiCredentials({
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
      }));
